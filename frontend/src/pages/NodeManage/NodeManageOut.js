...
import styles from './NodeManageOut.less';
...
<Route path='/show_data' component={ShowData}></Route>
...
const status = [ '无法使用', '可烧录', '运行中', '已关闭', '烧录中' ];
...
@Form.create()
class NodeManageOut extends PureComponent {
  state = {
    mapVisible: false,
    proVisible: [],
  ...
  columns = [
    ...
    },
    {
      ...
      render: val => {
        ...
        <Button icon="upload" type="primary" disabled={ val.status != 1 && val.status != 2 }>上传更新</Button>
    ...
    },
    {
      ...
      align:'center',
      render: val => {
        return <Button icon="info-circle" type="primary" disabled={ val.status == 0 || val.status == 3 } onClick={ () => this.handleShowData(val.MAC) }>展示数据</Button>;
      },
    },
    {
      title: '查看日志',
      align:'center',
      render: val => {
        return <Button icon="info-circle" type="primary" disabled={ val.status == 0 || val.status == 3 } onClick={ () => this.viewLog(val) }>查看日志</Button>;
      },
    },
  ];
  ...
  handleShowData = (id) => {
    var data = {dev_eui: id};
    var path = {
      pathname:'/node_manage/show_data',
      query: data,
    }
  ...
  menu = (
    <Menu onClick={this.onClickScenes}>
      <Menu.Item key="1">室内节点</Menu.Item>
      <Menu.Item key="2">室外节点</Menu.Item>
    </Menu>
  );
