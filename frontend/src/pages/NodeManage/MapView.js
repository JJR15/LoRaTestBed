...
import system_p from '../assets/system.png';
...
const CreateForm = Form.create()(props => {
  ...
  const buttons = [
    ...
    [
      ...
      <Button key="close" type="danger">
        关闭
      </Button>,
      <Button key="restart" type="danger">
        重启
      </Button>,
    ],
    [
...
class MapView extends PureComponent {
  ...
  addMap = payload => {
    var handleModalVisible = this.handleModalVisible,
      selectNode = this.selectNode,
      handleModalVisible2 = this.handleModalVisible2;
    let data = [];
    console.log("payload length:" + payload.nodeId.length);
    if (payload.code == 1) {
    ...
    //$("#allmap").css("width", (dom1.clientWidth)*0.5 + "px");
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].status == 1 || data[i].status == 2 || data[i].status == 4)
        myIcon = new BMap.Icon(node_success, new BMap.Size(32, 32), {
          anchor: new BMap.Size(15, 30),
        });
      else if (data[i].status == 3)
        myIcon = new BMap.Icon(node_close, new BMap.Size(32, 32), {
          anchor: new BMap.Size(15, 30),
        });
      else
        myIcon = new BMap.Icon(node_error, new BMap.Size(32, 32), {
          anchor: new BMap.Size(15, 30),
        });
    ...
    for (var i = 0; i < list0.status.length; i++) {
      var myIcon;
      if (list0.status[i] == 1)
        myIcon = new BMap.Icon(net_success, new BMap.Size(32, 32), {
          anchor: new BMap.Size(15, 30),
        });
  ...
  onClickScenes = key => {
    if (key.key == '1') {
      this.setState({
        display_indoor: 'block',
        display_outdoor: 'none',
      });
    } else {
      this.setState({
        display_indoor: 'none',
  ...
  menu = (
    <Menu onClick={this.onClickScenes}>
      <Menu.Item key="1">室内地图</Menu.Item>
      <Menu.Item key="2">室外地图</Menu.Item>
    </Menu>
  );
  ...
  componentWillMount() {
    ...
    $.ajax({
      // async: false,
      type: 'get',
      url: server_url + '/order/getoutallnode',
      xhrFields: {
        withCredentials: true,
      },
      crossDomain: true,
      ...
      success: payload => {
        ...
        $.getScript(
          'http://api.map.baidu.com/getscript?v=3.0&ak=ecTz4cxT7Ss7PXvycAukomg91sxCllWO&services=&t=20181029164750',
          function() {
            addMap(payload);
          }
        );
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="Node Map" content={content}>
        <Dropdown overlay={this.menu}>
          <a className="ant-dropdown-link" href="#">
            Choose Scene <Icon type="down" />
          </a>
        </Dropdown>
        <div
          style={{ background: '#fff', paddingTop: '20px', display: this.state.display_outdoor }}
        >
          <Content id="allmap" />
        </div>
        <div style={{ background: '#fff', paddingTop: '20px', display: this.state.display_indoor }}>
          <img alt="" width="100%" src={system_p} />
        </div>
        <CreateForm
          {...parentMethods}
