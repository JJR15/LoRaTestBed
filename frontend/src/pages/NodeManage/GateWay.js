...
class NodeManage extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      ...
      align:'center',
      render: val => {
        return <Button icon="caret-right" type="primary" disabled={ val.status != 0 } onClick={ () => this.operateNode(val.nodeId, 0) }>开启网关</Button>;
      },
    },
    {
      title: '网关关闭',
      align:'center',
      render: val => { 
        return <Button icon="pause-circle" type="danger" disabled={ val.status != 1} onClick={ () => this.operateNode(val.nodeId, 1) }>关闭网关</Button>;
      },
    },
    {
      title: '网关重启',
      align:'center',
      render: val => {
        return <Button icon="redo" type="normal" disabled={ val.status != 1} onClick={ () => this.operateNode(val.nodeId, 2) }>重启网关</Button>;
      },
    },
  ];
